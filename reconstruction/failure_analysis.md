...
**Cost.** The problem consumed roughly four to five days of effort across many sessions and was never resolved through the assistant. **[FIRST-HAND]**
...
I used Claude Opus 4.8 as an assistant during this deliverable. Its role was to help me locate the relevant sections within the four source papers, to map those sections to the relevant topics and stages of the memory-system problem, and to help structure this document. I also used it to critique and pressure-test my draft reasoning.

The substantive work is my own: the choice of application domain, the extraction of claims from each paper, all inferences drawn from those claims, the two constructed failure cases (X3 cross-user leakage and X4 sensitive-data admission), and the first-hand evidence from my Airflow/Fabric experience. I read the relevant sections of each source paper myself and verified every citation against the original PDFs.
